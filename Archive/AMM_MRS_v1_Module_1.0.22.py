...
class Filter:
    ...
    async def _create_memory(
        ...
        """
        try:
            # First, normalize the content to strip any existing tags/scores.
            normalized_content = self._normalize_memory_content(content)
            # Then, append the definitive importance score to the clean content.
            formatted_content = f"{normalized_content} [{importance:.1f}]"
    ...
    def _normalize_memory_content(content: str) -> str:
        """
        Normalizes memory content for robust comparison by repeatedly
        removing optional leading tags and trailing scores until the
        core content is isolated.
        """
        normalized_content = content.strip()
        # Loop until the string is no longer being changed
        while True:
            original_content = normalized_content
            # Remove a trailing score like '[0.5]' or '(0.5)'
            normalized_content = re.sub(
                r"\s*[\[\(]\s*\d+\.\d+\s*[\]\)]$", "", normalized_content
            ).strip()
            # Remove a leading tag like '[Hobby]'
            normalized_content = re.sub(
                r"^\s*\[\w+\]\s*", "", normalized_content
            ).strip()

            # If no changes were made, we've found the core content
    ...
    async def _delete_memory(self, exact_memory_text: str, __user__: dict) -> bool:
        """
        Delete a memory based on its content, accommodating different score formats.
        It first attempts an exact match. If that fails, it normalizes both
        the search text and the stored memories by removing any trailing score
        (e.g., `[0.5]` or `(0.5)`) and compares the base content.
        ...
        try:
            ...
            logger.info(
                f"Attempting to delete memory for user {user_id}: '{exact_memory_text}'"
            )
            ...
            for mem in memories:
                if mem.content == exact_memory_text:
                    memory_to_delete = mem
                    logger.info(f"Found exact match for memory ID: {mem.id}")
                    break

            # 2. If no exact match, try matching normalized content.
            if not memory_to_delete:
                normalized_search_text = self._normalize_memory_content(
                    exact_memory_text
                )
                for mem in memories:
                    normalized_mem_content = self._normalize_memory_content(mem.content)
                    if normalized_mem_content == normalized_search_text:
                        memory_to_delete = mem
                        logger.info(
                            f"Found normalized match for memory ID: {mem.id} ('{mem.content}')"
                        )
                        break
