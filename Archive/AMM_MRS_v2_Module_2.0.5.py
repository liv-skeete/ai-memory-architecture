...
class Filter:
    ...
    async def _identify_potential_memories(
        ...
        **kwargs,
        ...
        """
        await self._safe_emit("status", "🧠 Identifying potential memories...", **kwargs)
        ...
        response = await (
            self.query_openai_api(system_prompt, current_message)
            if self.valves.api_provider == "OpenAI API"
            else self.query_ollama_api(system_prompt, current_message)
        )

        if self.valves.verbose_logging:
            logger.info(f"Identification: Raw API response: {self._truncate_log_lines(response)}")

        raw_memories = self._parse_json_response(response)
        potential_memories = []

        for raw_mem in raw_memories:
            if isinstance(raw_mem, dict) and "content" in raw_mem and "importance" in raw_mem:
                try:
                    content_clean_score = self._strip_trailing_scores(raw_mem["content"])
                    content_clean, tag = self._strip_leading_tags(content_clean_score)
                    importance = float(raw_mem["importance"])

                    mem_obj = ProcessingMemory(content=content_clean, tag=tag, importance=importance)
                    potential_memories.append(mem_obj)

                except (ValueError, TypeError) as e:
                    logger.warning(f"Invalid memory format from LLM: {raw_mem} | Error: {e}")
    ...
    async def _identify_and_prepare_memories(
        ...
        **kwargs,
        ...
        important_memories = [
            ...
            if mem.importance and mem.importance >= self.valves.memory_importance_threshold
        ]
    ...
    async def _execute_memory_operations(
        ...
        **kwargs,
        ...
        sorted_ops = sorted(memory_operations, key=lambda x: x.importance or 0, reverse=True)
        ...
        for mem_op in sorted_ops:
            if mem_op.operation == "NEW" and mem_op.content:
                ...
                if created_id:
                    ...
                    citation_str = f"☑ {tag_str}{mem_op.content} [importance: {mem_op.importance:.1f}]"
                ...
                deleted_content = await self._delete_memory(mem_op, user_id, **kwargs)
                if deleted_content:
                    tag_str = f"{mem_op.tag} " if mem_op.tag else ""
                    citation_str = f"☒ {tag_str}{deleted_content}"
    ...
    async def _delete_memory(
        ...
        """
        Deletes a memory based on content, returning the original content on success.
        """
        try:
            if not user_id:
            ...
            for mem in all_memories:
                ...
                if db_content_no_tags.strip() == content_to_delete:
                    ...
                    # Return the original content that was matched for deletion
    ...
    async def _process_memory_storage(self, user_id: str, body: dict, relevant_memories: List[ProcessingMemory], **kwargs) -> None:
        """
        ...
        try:
            ...
            if memories_to_process:
                created_citations, deleted_citations = await self._execute_memory_operations(
                ...
                if all_citations:
                    ...
                    await self._send_citation(all_citations, user_id, "processed", **kwargs)

            else:
                logger.info(f"No new memory operations to execute for user {user_id}.")
                await self._safe_emit("status", "✅ Memories are up-to-date", **kwargs, final=True)

        except Exception as e:
            logger.error(f"Error during async memory processing: {e}", exc_info=True)
            await self._safe_emit("status", f"⚠️ Memory processing error", **kwargs, final=True)
        finally:
    ...
    async def _send_citation(self, status_updates: List[str], user_id: str, citation_type: str = "processed", **kwargs) -> None:
        """
    ...
    async def inlet(
        ...
        """
        if not body or not isinstance(body, dict) or not __user__ or not body.get("messages"):
            return body
        ...
        try:
            ...
            logger.info(f"Processing user message: {last_user_message_content[:100]}...")

            # --- Stage 1: Memory Retrieval (Synchronous) ---
            loop = asyncio.get_event_loop()
            db_memories = await loop.run_in_executor(None, Memories.get_memories_by_user_id, user_id)
            ...
            await self._safe_emit("status", "💭 Retrieving relevant memories...", **kwargs)

            relevant_memories = await self.get_relevant_memories(last_user_message_content, user_id, db_memories)

            if relevant_memories:
                formatted_memories, citation_memories = self._format_memories_for_context(relevant_memories)
                ...
                await self._send_citation(
                    citation_memories.split('\n')[1:], user_id, citation_type="read", **kwargs
                )
            else:
                logger.info("No relevant memories found for the current message.")
                await self._safe_emit("status", "🚫 No relevant memories found", **kwargs, final=True)
            ...
            if self.valves.async_memory_storage:
                ...
                task = asyncio.create_task(
                    self._process_memory_storage(user_id, body, relevant_memories, **kwargs)
                )
                self._background_tasks.add(task)
                task.add_done_callback(self._background_tasks.discard)
            else:
                await self._process_memory_storage(user_id, body, relevant_memories, **kwargs)
