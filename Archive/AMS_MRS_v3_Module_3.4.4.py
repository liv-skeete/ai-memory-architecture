...
changes:
  - v3.4.4 (2025-07-31):
      - **Refactor**: Removed the chunk number from warning logs related to individual memory validation to reduce potential confusion for administrators when multiple memories from the same chunk are discarded.
  - v3.4.3 (2025-07-31):
      - **Fix/Refactor**: Implemented a `Normalize-Compare-Preserve` validation strategy in the LLM re-ranking stage. The system now normalizes memories by stripping importance scores before sending them to the LLM, compares the LLM's normalized output against the original normalized content, and then preserves the original, verbatim memory for context injection. Importance scores are now also stripped from the final user-facing citations for a cleaner UI. This eliminates score bias, prevents validation errors, and maintains perfect data integrity.
  - v3.4.2 (2025-07-31):
      - **Security/Fix**: Implemented strict verbatim validation for all memories returned by the re-ranking LLM. The system now cross-references the LLM's output against the original candidate memories and discards any that are not a perfect match. This provides a robust defense against LLM hallucination, preventing modified or fabricated memories from being injected into the context and ensuring absolute data integrity.
  - v3.4.1 (2025-07-31):
      - **Fix**: Ensured memory content is handled verbatim during context injection and citation formatting. This prevents the accidental addition of formatting characters (e.g., markdown dashes) that corrupted memory content, which previously caused memory deletion to fail and citations to be displayed incorrectly.
  - v3.4.0 (2025-07-31):
  ...
  - v3.3.1 (2025-07-30):
      - **Fix**: When LLM search is disabled, the module now correctly returns all candidate memories from the vector search (`vector_search_top_k`) instead of incorrectly applying the `llm_search_top_k` limit.
  - v3.3.0 (2025-07-30):
      - **Feature**: Added `enable_llm_search` and `llm_search_top_k` valves to control LLM-based re-ranking.
      - **Refactor**: Removed `relevance_threshold` valve and its associated filtering logic in favor of a simpler top-k selection.
  - v3.2.1 (2025-07-26):
      - **Feature**: Re-added `num_ctx` as a configurable Ollama API parameter (`ollama_num_ctx`) to allow users to set the context window size. Default is 8192.
  - v3.2.0 (2025-07-25):
      - **Refactor**: Removed Stage 2 (LLM-based memory identification) processing entirely. The module now exclusively relies on assistant-led memory emission (`💾` and `🗑️`) for creation and deletion. The `inlet` is now solely for memory retrieval, and the `outlet` is solely for memory writing and pruning, creating a clear separation of concerns. This simplifies the architecture by removing three valves (`enable_stage_two_processing`, `identification_prompt`, `memory_importance_threshold`) and three core logic functions (`_identify_potential_memories`, `_identify_and_prepare_memories`, `_process_memory_storage`).
  - v3.1.7 (2025-07-25):
...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        ollama_num_ctx: int = Field(
            default=8192,
            description="Ollama context window size (num_ctx).",
        )
        # Common API settings
        ...
        include_assistant_message_in_vector_search: bool = Field(
            default=False,
            description="Toggle to include the assistant's message in the vector search query.",
        )
        ...
        enable_llm_search: bool = Field(
            default=True,
            description="Enable LLM-based search to re-rank and select the best memories from the candidates found by the vector search.",
        )
        ...
        llm_search_top_k: int = Field(
            default=10,
            description="The number of top-ranked memories to select after LLM re-ranking.",
        )
        rerank_chunk_size: int = Field(
            default=25,
            description="The size of each chunk for LLM re-ranking to avoid context overload.",
        )
    ...
    async def _get_relevant_memories_llm(
        ...
        validated 'Normalize-Compare-Preserve' strategy.
        ...
        tasks = []
        # Store maps of {normalized_content: original_content} for validation
        content_maps = []
        for i, chunk in enumerate(memory_chunks):
            # Create a map from score-stripped content to the original, raw content.
            # This allows us to validate against the semantics while preserving the original data.
            content_map = {
                self._strip_trailing_scores(mem.content): mem.content
                for mem in chunk
                if hasattr(mem, "content") and mem.content
            }
            content_maps.append(content_map)

            if not content_map:
                continue

            # Send only the normalized, score-stripped content to the LLM to avoid score bias.
            normalized_memories = list(content_map.keys())
            ...
            tasks.append(self._query_api(self.valves.api_provider, messages))

        # 3. Execute all tasks concurrently, with error handling
        api_responses = await asyncio.gather(*tasks, return_exceptions=True)

        # 4. Process and aggregate results from all chunks with the Normalize-Compare-Preserve strategy
        ...
        for i, response in enumerate(api_responses):
            ...
            # Get the content map for this chunk to validate against
            original_chunk_map = content_maps[i]
            if not original_chunk_map:
                continue
                ...
                if isinstance(memory_data, list):
                    for mem in memory_data:
                        if (
                            ...
                            # Normalize the content returned by the LLM
                            llm_content_normalized = self._strip_trailing_scores(
                                str(mem["content"])
                            )

                            # --- VALIDATE & PRESERVE ---
                            if llm_content_normalized in original_chunk_map:
                                # The memory is valid. Retrieve the original, verbatim content from the map.
                                original_content = original_chunk_map[
                                    llm_content_normalized
                                ]
                                try:
                                    relevance = float(mem["relevance"])
                                    all_reranked_memories.append(
                                        ProcessingMemory(
                                            content=original_content,  # Preserve original
                                            relevance=relevance,
                                        )
                                    )
                                except (ValueError, TypeError):
                                    logger.warning(f"Invalid relevance score: {mem}")
                            else:
                                if self.valves.verbose_logging:
                                    logger.warning(
                                        f"LLM hallucinated or modified memory. Discarding. Content: '{mem['content']}'"
                                    )
            except Exception as e:
                logger.error(f"Error processing LLM response for chunk {i+1}: {e}")
                logger.error(
                    f"Failed JSON content for chunk {i+1}: {prepared_response}"
                )
    ...
    async def get_relevant_memories(
        ...
        try:
            ...
            if self.valves.enable_vector_search:
                ...
                # Conditionally combine messages for the vector search query based on the valve.
                if self.valves.include_assistant_message_in_vector_search:
    ...
    def _format_memories_for_context(
        ...
        if relevant_memories:
            ...
            # Citations get a clean, score-stripped version for better readability
            citation_memories += "\n" + "\n".join(
                self._format_citation_line(
                    symbol="🔍",
                    content=self._strip_trailing_scores(mem.content),
                    score=mem.relevance,
    ...
    def _strip_trailing_scores(content: str) -> str:
        ...
        using the centralized regex pattern. It iteratively removes scores to handle multiple
        appended formats, which can be a defensive measure against LLM inconsistencies.
        ...
        """
        cleaned_content = content.strip()
        # Loop to remove multiple scores if the LLM appends them erroneously.
        while Filter._score_pattern.search(cleaned_content):
            cleaned_content = Filter._score_pattern.sub("", cleaned_content).strip()
