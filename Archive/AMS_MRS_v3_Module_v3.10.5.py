...
class Filter:
    ...
    # Hard-coded list of memory-enabled models (Stage 1)
    ...
    class Valves(BaseModel):
        ...
        memory_enabled_models: List[str] = Field(
            default_factory=lambda: ["sol", "sol-smart"],
            description="A list of model IDs that should have the MRS memory module activated.",
        )
    ...
    async def inlet(
        ...
        if model_id not in self.valves.memory_enabled_models:
            self._log_message(f"MRS inlet skipped for non-memory model: {model_id}")
            return body
        # --- End Gate ---
