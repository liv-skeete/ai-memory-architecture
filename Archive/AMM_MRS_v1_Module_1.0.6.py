...
class Filter:
    ...
    async def _identify_potential_memories(
        ...
        if potential_memories:
            ...
            # Log memory details for important memories (not just in verbose mode)
            for mem in potential_memories:
            ...
            # Log memory details only in verbose mode
            if self.valves.verbose_logging:
                for mem in potential_memories:
                    importance = mem.get("importance", 0)
                    if importance >= self.valves.memory_importance_threshold:
    ...
    async def inlet(
        ...
        try:
            ...
            truncated_message = last_user_message[:50] + "..." if len(last_user_message) > 50 else last_user_message
            ...
            if not relevant_memories:
                # Always log when no relevant memories are found
                ...
                # Return body unchanged when no memories found
                return body
            else:
                # We have relevant memories - format and update context
