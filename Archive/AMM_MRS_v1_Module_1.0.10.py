...
- v1.0.10 (2025-06-19):
  - Added `max_age` valve.
  - Added `importance_weight` valve.
  - Updated `_delete_excess_memories` to use these valves.
- v1.0.9 (2025-06-19):
  - Added `max_memories` and `selection_method` valve parameters for memory pruning.
  - Implemented `_delete_excess_memories` helper with FIFO and hybrid deletion logic based on age and importance.
- v1.0.8 (2025-05-31):
...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        max_age: int = Field(
            default=365,
            description="Maximum age in days for normalization",
        )
    ...
    async def _delete_excess_memories(self, user_id: str) -> None:
        ...
        mems = Memories.get_memories_by_user_id(user_id) or []
        logger.info(
        ...
        if self.valves.selection_method == "Oldest":
            ...
            def score(m):
                norm_age = min((now - m.updated_at) / (self.valves.max_age * 24 * 3600), 1)
                inv_age = 1 - norm_age
                match = re.search(r"\((\d+\.\d+)\)$", m.content)
                importance = float(match.group(1)) if match else 0.5
                return ((1 - self.valves.importance_weight) * inv_age) + (self.valves.importance_weight * importance)
        ...
        while len(mems) >= self.valves.max_memories:
            ...
            Memories.delete_memory_by_id_and_user_id(m.id, user_id)
            deleted += 1
