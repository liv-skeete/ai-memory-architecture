        ...
        while len(mems) > self.valves.max_memories:
            ...
            if self.vector_db_client:
                try:
                    ...
                    collection = self.vector_db_client.get_collection(
                    ...
                    logger.error(f"Failed to delete memory {m.id} from vector DB: {e}")
    ...
    async def _get_candidate_memories_vector_search(
        ...
        try:
            ...
            results = collection.query(
                query_embeddings=[query_embedding], n_results=top_k, include=["documents"]
            )
    ...
    async def get_relevant_memories(
        ...
        try:
            ...
            if self.valves.enable_vector_search:
                ...
                if not candidate_ids:
                    logger.info("No candidate memories found from vector search. Aborting.")
                    return []
                
                # Filter the full db_memories list to only include candidates from the vector search.
                # This ensures we use the fully-formatted memories from the canonical source (SQLite).
                candidate_id_set = set(candidate_ids)
                candidate_memories = [mem for mem in db_memories if mem.id in candidate_id_set]

                logger.info(f"Found {len(candidate_memories)} matching memories in database from {len(candidate_ids)} candidate IDs.")
