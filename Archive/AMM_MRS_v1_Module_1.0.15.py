...
class Filter:
    ...
    async def _identify_potential_memories(
        ...
        if self.valves.verbose_logging:
            ...
            logger.info(
                f"Relevant memories for identification: {[mem['content'] for mem in relevant_memories]}"
            )
    ...
    async def _execute_memory_creation(
        ...
        # Always log essential operations
        logger.info(
            f"Executing {len(memory_operations)} NEW memory creation operations"
        )
