...
changes:
  - v2.2.1 (2025-07-18):
      - Patched `_query_api` to apply `max_tokens` to Ollama payloads via `num_predict`, fixing potential JSON parsing failures with large messages.
      - Increased the default `max_tokens` from 750 to 4096 to better handle large Ollama responses.
  - v2.2.0 (2025-07-17):
      - Protected memories less than 1 day old from score-based deletion.
  - v2.1.9 (2025-07-17):
      - Added importance scores to deleted memory citations for UI consistency.
  - v2.1.8 (2025-07-17):
      - Grouped "Memories Processed" citation by operation type (Created, Deleted, Purged) and sorted each group by importance for clarity.
  - v2.1.7 (2025-07-16):
      - Added importance score to the essential log for purged memories.
  - v2.1.6 (2025-07-16):
      - Added essential logging for each purged memory, providing the same level of detail as create/delete operations.
      - Removed the redundant summary log for purged memory counts.
  - v2.1.5 (2025-07-13):
      - Patched `inlet` to correctly modify the message `body` in-place, ensuring multimodal image data is stripped for both synchronous (Stage 1) and asynchronous (Stage 2) processing, fully resolving the API error.
  - v2.1.4 (2025-07-13):
      - Patched `inlet` to gracefully handle multimodal messages by stripping image data and processing only text, preventing crashes with non-vision models.
      - Added `_sanitize_message_for_logging` to prevent raw image data from cluttering logs.
  - v2.1.3 (2025-07-13):
      - Patched `_safe_emit` to ensure status emissions are correctly suppressed when `show_status` is `False`.
  - v2.1.2 (2025-07-12):
      - Implemented robust logger configuration to guarantee correct, formatted handler is always present, restoring log prefixes.
      - Corrected and standardized logging to consistently differentiate between essential and verbose modes.
      - Added missing verbose logging for the Stage 2 (identification) prompt.
  - v2.1.0 (2025-07-11):
      - Corrected `inlet` logic to ensure memory storage (Stage 2) always runs, allowing new users to create their first memory.
      - Hardened the `close` method with a shared shutdown timeout to prevent resource leaks from long-running background tasks.
  - v2.0.9 (2025-07-10):
      - Implemented robust task cleanup in `close()` using `asyncio.gather` to ensure all background tasks are properly cancelled.
      - Added an early exit in `inlet()` to prevent processing when no memories exist for the user.
  - v2.0.8 (2025-07-10):
    ...
    async def _process_memory_storage(
        ...
        **kwargs,
        ...
        try:
            ...
            purged_citations = await self._delete_excess_memories(user_id, **kwargs)
            ...
            # Consolidate all memory operation citations for a single message
            ...
            if all_citations:
                ...
                def get_importance_from_citation(citation: str) -> float:
                    ...
                    # This regex now correctly handles importance scores from all operations.
                ...
                created_citations.sort(
                    key=get_importance_from_citation, reverse=True
                )
                deleted_citations.sort(
                    key=get_importance_from_citation, reverse=True
                )

                # Combine the sorted lists, preserving grouping (Created, Deleted, Purged)
                final_citations = (
