...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        dedupe_distance_threshold: float = Field(
            default=0.1,
            description="Distance threshold for considering memories near-duplicates (Chroma distance)",
        )
    ...
    async def _delete_memory(
        ...
        try:
            ...
            if ids and distances and distances[0] < self.valves.delete_distance_threshold:
    ...
    async def _deduplicate_user_memories(self, user_id: str, **kwargs) -> List[str]:
        """
        Find connected components of near-duplicate memories using vector distances
        and consolidate each cluster into a single canonical memory.
        This implementation hardens array/list handling to avoid NumPy truth-value ambiguity.
        """
        citations: List[str] = []
        # Preconditions
        if (self.vector_db_client is None) or (not bool(self.valves.enable_deduplication)):
            return citations

        loop = asyncio.get_event_loop()
        # Load all memories from primary DB (for retention scores and importances)
        db_mems = (
            await loop.run_in_executor(None, Memories.get_memories_by_user_id, user_id)
            or []
        )
        ...
        # Standard log entry to mirror existing process start logs
        ...
        try:
            ...
            # Explicitly normalize possibly numpy/array-like containers to Python lists
            def _as_list(x):
                if x is None:
                    return []
                if hasattr(x, "tolist"):
                    return x.tolist()
                return list(x) if hasattr(x, "__iter__") and not isinstance(x, (str, bytes, dict)) else []
            ...
            # Build valid query set (ids with embeddings) with safe coercions
            id_to_doc: Dict[str, str] = {}
            query_ids: List[str] = []
            query_embs: List[List[float]] = []
            limit = min(len(ids), len(embs))
            for i in range(limit):
                mid = ids[i]
                emb = embs[i]

                # Coerce id to str for consistent keys
                mid_str = str(mid)

                # Normalize embedding to list-of-floats
                if emb is None:
                    continue
                e = emb.tolist() if hasattr(emb, "tolist") else (list(emb) if hasattr(emb, "__iter__") else None)
                if not isinstance(e, list) or len(e) == 0:
                    continue

                query_ids.append(mid_str)
                query_embs.append([float(v) for v in e])

                # Map document if present
                if i < len(docs) and docs[i] is not None:
                    id_to_doc[mid_str] = docs[i]
            ...
            # Single multi-query call
            results = collection.query(
                query_embeddings=query_embs,
                n_results=int(self.valves.vector_search_top_k),
                include=["distances"],
            )

            res_ids_raw = results.get("ids", [])
            res_dists_raw = results.get("distances", [])

            # Normalize query results to nested Python lists
            res_ids: List[List[str]] = []
            for arr in _as_list(res_ids_raw):
                res_ids.append([str(x) for x in _as_list(arr)])

            res_dists: List[List[float]] = []
            for arr in _as_list(res_dists_raw):
                vals = []
                for x in _as_list(arr):
                    try:
                        # If x is array-like, pick scalar
                        ...
                        continue
                res_dists.append(vals)

            # Build undirected adjacency among near-duplicates
            thr = float(self.valves.dedupe_distance_threshold)
            adjacency: Dict[str, set] = {}
            min_edge_distance: Optional[float] = None

            for i, qid in enumerate(query_ids):
                neighbors = res_ids[i] if i < len(res_ids) else []
                dists = res_dists[i] if i < len(res_dists) else []
                for nid, dist_val in zip(neighbors, dists):
                    nid_str = str(nid)
                    if nid_str == qid:
                    ...
                    try:
                        ...
                        continue
                    if dv <= thr:
                        adjacency.setdefault(qid, set()).add(nid_str)
                        adjacency.setdefault(nid_str, set()).add(qid)
                        if (min_edge_distance is None) or (dv < min_edge_distance):
                            min_edge_distance = dv

            # Extract connected components with size >= 2
            visited: set = set()
            clusters: List[set] = []
            ...
            # Map DB ids to memory objects (ids are strings in DB)
            db_map: Dict[str, Any] = {str(m.id): m for m in db_mems}

            # Log number of clusters and, in verbose mode, a concise preview before any deletions
            try:
                cluster_count = len(clusters)
                verbose_lines: List[str] = []
                if self.valves.verbose_logging:
                    for idx, comp in enumerate(clusters, 1):
                        # Preview up to 3 members' canonical texts
                        previews: List[str] = []
                        for mid in list(comp)[:3]:
                            text = db_map.get(mid).content if mid in db_map else id_to_doc.get(mid, "")
                            canonical = self._strip_score(text)
                            previews.append(self._format_log_content(canonical))
                        line = f"Cluster {idx} (size {len(comp)}): " + "; ".join(previews)
                        verbose_lines.append(line)
                distance_suffix = f" — Vector distance {min_edge_distance:.3f}" if 'min_edge_distance' in locals() and (min_edge_distance is not None) else ""
                self._log_message(
                    f"Found {cluster_count} duplicate cluster(s){distance_suffix}",
                    verbose_msg="\n".join([f"  └─ {ln}" for ln in verbose_lines]) if verbose_lines else None,
                )
            except Exception:
                # Logging must not interfere with dedupe execution
                pass

            for comp in clusters:
                members = [mid for mid in comp if (mid in db_map or mid in id_to_doc)]
                if len(members) < 2:
                    continue

                # Keeper: argmax retention score
                def retention(mid: str) -> float:
                    m = db_map.get(mid)
                    if m is None:
                        # Fallback object to de-prioritize items missing in primary DB
                        stub = type(
                            "obj",
                            (object,),
                            {
                ...
                # Average importance over cluster
                imps: List[float] = []
                for mid in members:
                    m = db_map.get(mid)
                    if m is not None:
                        imps.append(self._get_memory_importance(m))
                    else:
                        doc = id_to_doc.get(mid, "")
                        imps.append(
                            self._get_memory_importance(
                                type("obj", (object,), {"content": doc})()
                            )
                        )
                avg_importance = sum(imps) / len(imps) if len(imps) > 0 else 0.5

                # Keeper's canonical content (score stripped)
                if keeper_id in db_map:
                    keeper_text = db_map[keeper_id].content
                else:
                    keeper_text = id_to_doc.get(keeper_id, "")
                canonical = self._strip_score(keeper_text)

                # Delete all members (including would-be keeper)
                for mid in members:
                    await self._delete_memory_by_id(mid, user_id)

                # Recreate canonical keeper with averaged importance
                await self._create_memory(
                    ProcessingMemory(
                        content=canonical, importance=avg_importance, operation="NEW"
                    ),
                    user_id,
                    **kwargs,
                )
                ...
                self._log_message(
                    f"Deduplicated cluster of {len(members)} → canonical reinserted",
                    verbose_msg=f"  └─ Canonical: {self._format_log_content(keeper_text)} | Avg importance: {avg_importance:.2f}",
                )
