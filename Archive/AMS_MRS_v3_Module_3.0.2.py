...
class Filter:
    ...
    async def _backfill_user_vectors(self, user_id: str, **kwargs) -> None:
        ...
        try:
            ...
            collection_name = f"user-memory-{user_id}"
            collection = self.vector_db_client.get_or_create_collection(name=collection_name)
            ...
            if missing_in_vector_db:
                ...
                await self._safe_emit("status", f"⚙️ Backfilling {count} vector(s)...", **kwargs)
                ...
                await self._safe_emit("status", "✅ Vector backfill complete.", **kwargs)
            else:
            ...
            await self._safe_emit("status", "⚠️ Vector backfill error.", **kwargs, final=True)
