...
class Filter:
    ...
    async def _delete_excess_memories(
        ...
        if perform_percentage_prune:
            logger.info(f"Starting deep pruning check for user {user_id}...")
        ...
        if num_to_prune <= 0:
            if perform_percentage_prune:
                logger.info(
                    f"Deep prune check for user {user_id}: No memories meet pruning criteria. (Total: {num_mems}, Threshold: {self.valves.pruning_threshold})"
                )
        ...
        logger.info(
            f"User {user_id} has {num_mems} memories; pruning {num_to_prune} using {self.valves.selection_method} method."
        )
        ...
        for line in lines:
            ...
            if match:
                ...
                if op_symbol == "💾":
                    ...
                    # Use the existing regex to create clean content by removing the score.
                    clean_content = Filter._score_pattern.sub("", content).strip()
    ...
    async def _run_inlet_pruning_task(self, user_id: str, **kwargs) -> None:
        """
        A background task wrapper for the inlet's deep-pruning process that
        handles sending a citation if memories are purged.
        """
        try:
            ...
            logger.error(f"Error in inlet pruning task: {e}", exc_info=True)
            await self._safe_emit(
                "status", "⚠️ Pruning task error.", **kwargs, final=True
            )
    ...
    async def inlet(
        ...
        try:
            ...
            # On the first turn of a session, run maintenance tasks like vector backfilling
            # and deep memory pruning concurrently in the background.
            if user_id not in self._session_init_completed_users:
                self._session_init_completed_users.add(user_id)
                logger.info(
                    f"First turn for user {user_id} this session. Kicking off maintenance tasks."
                )

                # Task 1: Vector DB integrity check.
                backfill_task = asyncio.create_task(
                    self._backfill_user_vectors(
                        user_id, __event_emitter__=__event_emitter__
                    )
                )
                self._background_tasks.add(backfill_task)
                backfill_task.add_done_callback(self._background_tasks.discard)

                # Task 2: Deep memory pruning (percentage-based) with citation.
                prune_task = asyncio.create_task(
                    self._run_inlet_pruning_task(
                        user_id, __event_emitter__=__event_emitter__
                    )
                )
                self._background_tasks.add(prune_task)
                prune_task.add_done_callback(self._background_tasks.discard)
