...
changes:
  - v3.8.3 (2025-08-04):
      - **Fix**: Resolved a critical race condition during session initialization where concurrent pruning and vector backfilling could cause data corruption. These tasks now run sequentially (pruning first, then backfill) to ensure data integrity.
  - v3.8.2 (2025-08-04):
      - **Refactor**: Centralized all memory deletion logic into a single, atomic-safe `_delete_memory_by_id` method. This new method deletes from the vector database *before* the primary database, preventing orphaned vector records.
      - **Fix**: Corrected a critical architectural flaw where deletion operations in `_delete_memory` (for `🗑️` commands) and `_delete_excess_memories` (for pruning) were not atomic, leading to data inconsistencies when a vector DB deletion failed after a primary DB deletion succeeded. Both functions now use the new centralized method.
  - v3.8.1 (2025-08-04):
      - **Fix**: Implemented a robust, centralized fix to ensure final status messages are always cleared from the UI. The `_memory_writer_task` now handles all terminal states (`complete` or `no operations`) by displaying a message, waiting with a direct `asyncio.sleep`, and then clearing it. This simpler, more direct approach resolves inconsistent UI behavior without the complexity of background tasks.
  - v3.8.0 (2025-08-04):
      - **Refactor**: Rearchitected the memory deletion mechanism to use a fast, safe, and scalable vector-based lookup instead of iterating through the database. This new approach generates an embedding for the canonical (score-stripped) content to be deleted, performs a `top_k=1` vector search, and only proceeds with deletion if a near-identical match (distance < 0.001) is found. This eliminates slow loops and prevents accidental deletions caused by minor content mismatches.
      - **Feat**: Enhanced memory creation by storing a canonical embedding (based on score-stripped content) in the vector database, while keeping the original, full content in the document for accurate citation and display.
      - **Fix**: Corrected a critical bug in the `_backfill_user_vectors` function where embeddings were incorrectly generated from non-canonical content (including the score), causing vector mismatches during deletion lookups. The backfill process now correctly uses the canonical, score-stripped content for embedding.
      - **Fix**: Added verbose-only diagnostic logging to the deletion process to show the requested vs. retrieved memory content when a vector match fails, aiding in debugging similarity issues.
 - v3.7.6 (2025-08-04):
      - **Fix**: Corrected a `NameError` in `_parse_assistant_response_for_memories` caused by incorrectly calling a static method with `self`.
  - v3.7.5 (2025-08-04):
      - **Refactor**: Centralized the logic for stripping importance scores from memory content into a single `_strip_score` helper method. This improves code reuse and maintainability.
  - v3.7.4 (2025-08-04):
      - **Fix**: Resolved an issue where memory deletions would fail due to overly strict matching. The deletion logic now canonicalizes both the request and the stored memory by stripping importance scores before comparison, making the process more robust. Also improved logging for failed deletions.
  - v3.7.3 (2025-08-03):
      - **Refactor**: Overhauled the status message emission system to be more robust and reliable. Replaced scattered `_safe_emit` calls with a centralized `_emit_status` function and wrapped the `inlet` and `outlet` logic in `try...finally` blocks to guarantee that every status message is correctly cleared. This resolves issues with stuck or dangling status notifications.
  - v3.7.2 (2025-08-03):
      - **Fix**: Resolved a race condition where memory processing status messages could get stuck. The `_safe_emit`, `_finalize_memory_processing`, and `_backfill_user_vectors` functions were updated to ensure status updates are correctly flagged as "final," guaranteeing proper UI dismissal.
  - v3.7.1 (2025-08-03):
      - **Refactor**: Increased memory score precision to two decimal places for both storage and display, providing more nuanced scoring.
  - v3.7.0 (2025-08-02):
      - **Refactor**: Standardized the entire logging system to improve clarity and consistency. Introduced centralized helper methods (`_log_message`, `_format_log_content`) to manage standard vs. verbose output, ensuring uniform formatting for all log entries. Removed the `max_log_lines` valve to simplify configuration.
  - v3.6.3 (2025-08-01):
      - **Refactor**: Removed the assistant's message from the vector search query to improve search accuracy by focusing solely on the user's intent. The query now only consists of the user's message.
  - v3.6.2 (2025-08-01):
      - **Feature**: Rearchitected memory pruning to use a dual-trigger system. A deep, percentage-based prune now runs once per session (triggered from the `inlet`), allowing for long-term corpus growth. A shallow, hard-limit prune runs on every turn (triggered from the `outlet`) as a safety net. This prevents overly aggressive purging while still controlling memory size.
  - v3.6.1 (2025-08-01):
      - **Fix**: Corrected a critical issue where memories were created with duplicate importance scores, which prevented them from being found by delete operations. The `_parse_assistant_response_for_memories` function now strips scores from content before passing it to the creation pipeline, ensuring a canonical format.
  - v3.6.0 (2025-08-01):
      - **Feature**: Implemented a hybrid pruning mechanism that combines a fixed upper limit (`max_memories`) with a percentage-based approach (`pruning_percentage`, `pruning_threshold`). This allows for more dynamic memory management, especially for smaller memory collections, while still enforcing a hard ceiling.
  - v3.5.2 (2025-08-01):
...
class Filter:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self._log_message("ChromaDB client initialized.")

            self._log_message("Loading embedding model: all-MiniLM-L6-v2")
            self.embedding_model = SentenceTransformer("all-MiniLM-L6-v2")
            self._log_message("Embedding model loaded.")
        except Exception as e:
            logger.error(
        ...
        for key, value in new_valves.items():
            if hasattr(self.valves, key):
                ...
                if key.endswith("_prompt") and isinstance(value, str):
                    ...
                    self._log_message(
                        f"Updating {key}", verbose_msg=f"  └─ New value: {preview}"
                    )
                else:
                    self._log_message(
                        f"Updating {key}", verbose_msg=f"  └─ New value: {value}"
                    )
        ...
        try:
            ...
            if self.vector_db_client:
                ...
                if collection.get(ids=[memory_id]).get("ids"):
                    ...
                    self._log_message(
                        None,
                        verbose_msg=f"Deleted vector {memory_id} from '{collection_name}'",
                    )
    ...
    async def _delete_excess_memories(
        ...
        for m in mems_to_prune:
            ...
            if success:
                ...
                self._log_message(
                    f"Purged memory: {self._format_log_content(m.content)}",
        ...
        for mem_op in sorted_ops:
            if mem_op.operation == "NEW" and mem_op.content:
                ...
                if deleted_content:
                    ...
                    self._log_message(
                        f"Deleted memory: {self._format_log_content(deleted_content)}",
                        verbose_msg=f"  └─ Citation: {citation_str}",
                    )
            ...
            if memory and hasattr(memory, "id"):
                ...
                if self.vector_db_client and self.embedding_model:
                    try:
                        ...
                        self._log_message(
                            f"Added vector for memory {memory.id} to '{collection_name}'"
                        )
                    except Exception as e:
                        logger.error(
    ...
    async def _delete_memory(
        ...
        try:
            ...
            query_embedding = self.embedding_model.encode(content_to_delete).tolist()
            ...
            if ids and distances and distances[0] < 0.001:
                ...
                if success:
                    ...
                    return original_content, None, importance_score
            else:
    ...
    def _parse_assistant_response_for_memories(
        ...
        for line in lines:
            ...
            if match:
                ...
                if op_symbol == "💾":
                    # For new memories, extract the importance score and strip it from the content.
                    importance = Filter._get_memory_importance(
                        type("obj", (object,), {"content": content})()
                    )
                    # Create clean, canonical content by stripping the score.
                    clean_content = Filter._strip_score(content)
                    ...
                    # For deletions, canonicalize the content by stripping the score to make matching robust.
                    clean_content = Filter._strip_score(content)
                    operations_found.append(
        ...
        try:
            ...
            if missing_in_vector_db:
                ...
                embeddings = await asyncio.to_thread(
                    self.embedding_model.encode, canonical_contents
                )
    ...
    async def _run_session_init_tasks(self, user_id: str, **kwargs) -> None:
        ...
        """
        self._log_message(
            f"Running sequential session init tasks for {user_id}: Pruning then Backfill"
        )
