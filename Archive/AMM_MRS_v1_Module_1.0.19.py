...
changes:
  - v1.0.19 (2025-06-29):
      - Refactored Stage 2 memory processing to be fully non-blocking
  - v1.0.18 (2025-06-29):
      - Removed diagnostic regex and logging from DELETE operation processing in `_identify_and_prepare_memories`
      - Simplified DELETE branch to use direct slice for content extraction
  - v1.0.17 (2025-06-29):
      - Fixed relevant_memories logging to consistently show memory scores
      - Enhanced memory logging in identification stage
      - Ensured memory content with scores appears in logs as passed to LLM
  - v1.0.16 (2025-06-29):
      - Implemented memory deletion operation processing
      - Renamed _execute_memory_creation to _execute_memory_operations to handle both creation and deletion
      - Added _delete_memory method to execute memory deletions based on exact memory text
      - Updated status reporting to handle delete operations
  - v1.0.15 (2025-06-26):
      - Removed unsafe __del__ destructor and enhanced resource cleanup
      - Added context manager pattern (__aenter__/__aexit__) for safer resource handling
      - Improved close() method with timeouts for task cancellation and session closure
      - Cleaned up aiohttp connector properly
  - v1.0.14 (2025-06-23):
      - Removed redundant 'enabled' valve parameter and corresponding code checks; memory toggling now controlled at application level.
  - v1.0.13 (2025-06-20):
      - Implemented configurable exponential decay for memory recency scoring.
      - Repurposed `max_age` valve parameter to control half-life for decay function.
      - Enhanced hybrid memory selection to prioritize memories using decay model.

  - v1.0.11 (2025-06-19):
      - Improved performance with non-blocking event emitters for all non-critical operations.
      - Added comprehensive memory pruning with configurable parameters.
      - Implemented hybrid deletion strategy using both age and importance scores.

  - v1.0.7 (2025-05-31):
      - Integrated Stage 1 and Stage 2 processing for better context awareness.
      - Added bidirectional flow of memory context between retrieval and storage stages.
      - Ensured identification stage runs regardless of relevance stage results.

  - v1.0.6 (2025-05-28):
      - Optimized logging system to reduce noise while preserving critical operational visibility.
      - Fixed logical errors in memory processing flow that affected log visibility.

  - v1.0.4 (2025-05-20):
      - Standardized memory formatting and citation display across module functions.
      - Consolidated status emission patterns between processing stages.
...
class Filter:
    ...
    @staticmethod
    def _score(m, importance_weight: float, half_life_days: float) -> float:
        inv_age = Filter._inv_age_exponential(m.updated_at, half_life_days)
        match = re.search(r"\[(\d+\.\d+)\]$", m.content)
        importance = float(match.group(1)) if match else 0.5
        return (importance_weight * importance) + ((1 - importance_weight) * inv_age)

    async def _delete_excess_memories(self, user_id: str) -> None:
        """Remove excess memories based on max_memories and selection_method."""
        ...
        logger.info(
            f"User {user_id} has {len(mems)} memories; max allowed is {self.valves.max_memories}"
        )
        if len(mems) < self.valves.max_memories:
            logger.info(f"No excess memories to delete for user {user_id}")
            return
        logger.info(f"Selection method: {self.valves.selection_method}")
        if self.valves.selection_method == "Oldest":
            mems.sort(key=lambda m: m.created_at)
            logger.info("Pruning using Oldest (oldest first)")
        else:
            logger.info("Pruning using Hybrid (age & importance)")
            half_life_days = self.valves.max_age
            mems.sort(
                key=lambda m: self._score(
        ...
        while len(mems) >= self.valves.max_memories:
            m = mems.pop(0)
            logger.info(f"Deleting memory {m.id} for user {user_id}")
            await loop.run_in_executor(
                None, Memories.delete_memory_by_id_and_user_id, m.id, user_id
            )
            deleted += 1
        logger.info(f"Deleted {deleted} excess memories for user {user_id}")
    ...
    def _format_memory_item(
        ...
        return f"☐ {content} [{score_label}: {score:.1f}]"
    ...
    def _format_memories_for_context(
        ...
        if relevant_memories:
            ...
            citation_memories += "\n" + "\n".join(
                self._format_memory_item(mem["content"], mem["score"])
    ...
    def _format_relevant_memories(self, memories: List[Dict[str, Any]]) -> str:
        ...
        Returns:
            Formatted string of memories with scores in chronological order (newest first)
        """
        # Reverse order to show newest memories first
        return self.format_bulleted_list(
            [(mem["content"] + f" [{mem['score']:.1f}]") for mem in reversed(memories)]
        )
    ...
    async def _identify_potential_memories(
        ...
        if self.valves.verbose_logging:
            # Log memory contents with scores
            logger.info(
                "Relevant memories for identification: %s",
                [f"{mem['content']} [{mem['score']:.1f}]" for mem in relevant_memories],
            )
        ...
        if potential_memories:
            ...
            for mem in potential_memories:
                ...
                if importance >= self.valves.memory_importance_threshold:
                    logger.info(
                        f"Memory content: {mem.get('content', '')} [score: {importance:.2f}]"
                    )

            # Log full breakdown only in verbose mode
            if self.valves.verbose_logging:
                logger.info(
                    f"Found {len(potential_memories)} potential memories ({above_threshold} above threshold, {below_threshold} below)"
                )
        else:
            # Essential logging (always shown)
    ...
    async def _identify_and_prepare_memories(
        self, current_message: str, relevant_memories: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:  # Returns list of memory operations (NEW or DELETE)
        """
        ...
        Args:
            ...
            relevant_memories: List of relevant memories from Stage 1
        Returns:
            List of memory operations for memories meeting the importance threshold.
            Operations can be either NEW (create) or DELETE (remove).
        ...
        if not important_memories:
            ...
            logger.info(
                f"Identified {len(important_memories)} important potential memories meeting threshold ({self.valves.memory_importance_threshold:.2f})"
            )

            # Create operations for the important memories (NEW or DELETE)
            memory_operations = []
            for mem in important_memories:
                ...
                if content.startswith("[Delete] "):
                    delete_text = content[len("[Delete] ") :]
            ...
            op_summary_parts = []
            if new_ops:
                op_summary_parts.append(
                    ", ".join(f"NEW({op.get('importance', 0):.2f})" for op in new_ops)
                )
            if delete_ops:
                op_summary_parts.append(
                    ", ".join(
                        f"DELETE({op.get('importance', 0):.2f})" for op in delete_ops
                    )
                )
    ...
    async def _execute_memory_operations(
        ...
        Args:
            memory_operations: List of operations from _identify_and_prepare_memories
            __user__: The user dictionary object
            __event_emitter__: Optional event emitter for sending status updates
        """
        # Count operations by type
        new_ops = [op for op in memory_operations if op.get("operation") == "NEW"]
        delete_ops = [op for op in memory_operations if op.get("operation") == "DELETE"]

        # Always log essential operations
        logger.info(
            f"Executing {len(new_ops)} NEW creation and {len(delete_ops)} DELETE operations"
        )
        if not memory_operations:
            return

        # Store status messages with importance for sorting
            ...
            if operation_type == "NEW":
                if content:
                    ...
                    if self.valves.verbose_logging:
                        ...
                        logger.info(
                            f"Creating memory for user {__user__.get('id', 'UNKNOWN')} (Importance: {importance:.2f}): {truncated_content}"
                        )
                    memory_id = await self._create_memory(content, __user__, importance)
                    if memory_id:
                        # Use the same formatting method as for memory retrieval
                        status = f"☑ {content} [importance: {importance:.1f}]"
                        ...
                        status = "☒ Failed to create new memory"
                    ...
                    status_updates_with_importance.append(
                        ("☒ Skipped creating memory: missing content", 0)
                    )
            elif operation_type == "DELETE":
                if content:
                    # Log deletion with truncated content
                    if self.valves.verbose_logging:
                        truncated_content = (
                            content[:100] + "..." if len(content) > 100 else content
                        )
                        logger.info(
                            f"Deleting memory for user {__user__.get('id', 'UNKNOWN')}: {truncated_content}"
                        )

                    # Execute the delete operation
                    success = await self._delete_memory(content, __user__)
                    ...
                    if success:
                        ...
                        status = f"☒ Failed to delete memory: {content}"
                        logger.warning(status)
                        status_updates_with_importance.append((status, 0))
                else:
                    logger.warning("Skipping DELETE operation: Content is missing")
                    status_updates_with_importance.append(
                        ("☒ Skipped deleting memory: missing content", 0)
                    )
            else:
                # This case should not happen after refactoring
                logger.warning(
                    f"Skipping unexpected operation type '{operation_type}': {op}"
                )
                # Unknown operations get importance 0 for sorting
                status_updates_with_importance.append(
                    (f"☒ Skipped unknown operation: {operation_type}", 0)
                )
    ...
    async def _create_memory(
        ...
        try:
            ...
            memory = await loop.run_in_executor(
                None, Memories.insert_new_memory, __user__.get("id"), formatted_content
            )
    ...
    async def _delete_memory(self, exact_memory_text: str, __user__: dict) -> bool:
        """
        Delete a memory based on exact memory text.

        Args:
            exact_memory_text: The exact text of the memory to delete
            __user__: The user dictionary object
        ...
        try:
            ...
            logger.info(f"Looking for memory with content: '{exact_memory_text}'")

            # Parse memory parts from search text
            search_base_content = exact_memory_text
            search_score = None

            # Extract score if present in the format [0.9]
            score_match = re.search(r"\[(\d+\.\d+)\]$", exact_memory_text)
            if score_match:
                search_score = float(score_match.group(1))
                # Extract base content (everything before the score)
                search_base_content = exact_memory_text[
                    : exact_memory_text.rfind("[")
                ].strip()
                logger.info(
                    f"Parsed search text - Base: '{search_base_content}', Score: {search_score}"
                )
            ...
            for memory in memories:
                ...
                memory_score = None
                memory_base_content = memory_content

                # Extract score if present
                mem_score_match = re.search(r"\[(\d+\.\d+)\]$", memory_content)
                if mem_score_match:
                    memory_score = float(mem_score_match.group(1))
                    memory_base_content = memory_content[
                        : memory_content.rfind("[")
                    ].strip()

                # Log comparison for first 3 memories
                if len(close_matches) < 3:
                    logger.info(
                        f"Comparing memory: Base: '{memory_base_content}', Score: {memory_score}"
                    )
                    logger.info(
                        f"Against search: Base: '{search_base_content}', Score: {search_score}"
                    )

                # 1. Exact full content match (original behavior)
                if memory_content == exact_memory_text:
                    memory_to_delete = memory
                    logger.info(
                        f"Found exact full content match for memory: {memory.id}"
                    )
                    break

                # 2. Exact base content match (ignoring score)
                elif memory_base_content == search_base_content:
                    # If both have scores, check if they're within 0.01 of each other
                    if memory_score is not None and search_score is not None:
                        if abs(memory_score - search_score) < 0.01:
                            memory_to_delete = memory
                            logger.info(
                                f"Found match with identical base content and similar score: {memory.id}"
                            )
                            break
                        else:
                            # Same content but different score, record as close match
                            close_matches.append(
                                (
                                    memory,
                                    f"same content, different score: {memory_score} vs {search_score}",
                                )
                            )
                    else:
                        # One or both don't have scores, but base content matches
                        memory_to_delete = memory
                        logger.info(
                            f"Found match with identical base content: {memory.id}"
                        )
                        break

                # 3. Fuzzy base content match - normalized case & whitespace
                elif (
                    memory_base_content.lower().strip()
                    == search_base_content.lower().strip()
                ):
                    close_matches.append((memory, "case/whitespace differences"))

            # If no exact match but we have close matches, use the first one
            if not memory_to_delete and close_matches:
                memory_to_delete = close_matches[0][0]
                reason = close_matches[0][1]
                logger.info(
                    f"Using close match ({reason}) for memory: {memory_to_delete.id}"
                )
                logger.info(f"Memory content: '{memory_to_delete.content}'")

            if not memory_to_delete:
                logger.warning(f"No memory found matching: '{exact_memory_text}'")
                # Show sample of available memories for debugging
                logger.warning("Available memories (up to 3):")
                for i, memory in enumerate(memories[:3]):
                    logger.warning(f"{i+1}. '{memory.content}'")
                return False
            ...
            if result:
                logger.info(
                    f"Successfully deleted memory {memory_to_delete.id} for user {user_id}: '{memory_to_delete.content}'"
                )
                return True
            else:
                logger.warning(
                    f"Failed to delete memory {memory_to_delete.id} for user {user_id}"
                )
                return False
    ...
    async def _process_memory_storage(
        ...
        # Initialize memory_operations for the finally block
        memory_operations = []
        ...
        try:
            ...
            if self.valves.show_status and __event_emitter__:
                # Determine appropriate message based on operations performed
                new_ops_count = len(
                    [op for op in memory_operations if op.get("operation") == "NEW"]
                )
                delete_ops_count = len(
                    [op for op in memory_operations if op.get("operation") == "DELETE"]
                )

                if new_ops_count > 0 and delete_ops_count > 0:
                    description = f"☑ Saved {new_ops_count} memories and deleted {delete_ops_count} memories"
                elif new_ops_count > 0:
                    description = f"☑ Saved {new_ops_count} important memories [threshold: {self.valves.memory_importance_threshold:.1f}]"
    ...
    async def _send_citation(
        ...
        Args:
            ...
            user_id: The user ID
            citation_type: Type of citation ("saved", "read", or "deleted")
        """
