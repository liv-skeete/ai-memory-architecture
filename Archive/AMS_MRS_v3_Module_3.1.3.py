...
class Filter:
        ...
        **kwargs,
        ...
        try:
            ...
            await self._finalize_memory_processing(
                created_citations, deleted_citations, purged_citations, user_id, **kwargs
            )
    ...
    async def _process_assistant_memories(
        self,
        memory_operations: List[ProcessingMemory],
        user_id: str,
        **kwargs,
        ...
        """
        Asynchronously processes assistant-derived memory operations. This is the
        outlet-based workflow, which does not handle memory purging.
        """
        try:
            logger.info(f"Starting async processing for assistant-derived memories for user: {user_id}")

            created_citations, deleted_citations = await self._execute_memory_operations(
                memory_operations, user_id, **kwargs
            )

            # Call the centralized helper, passing an empty list for purged citations.
            await self._finalize_memory_processing(
                created_citations, deleted_citations, [], user_id, **kwargs
            )

        except Exception as e:
            logger.error(f"Error during assistant memory processing: {e}", exc_info=True)
            await self._safe_emit(
                "status", f"⚠️ Memory processing error", **kwargs, final=True
            )
    ...
    async def outlet(
        ...
        """
        Process the assistant's final response. If Stage 2 processing was
        disabled in the inlet, this function will parse the assistant's
        response for specially marked memory content and save it.
        """

        # If Stage 2 is enabled, this function is a pass-through.
        if self.valves.enable_stage_two_processing:
            if self.valves.verbose_logging:
                logger.info("Stage 2 enabled, outlet is a pass-through.")
            return body
        ...
        try:
            ...
            # Parse the response for memory operations (create or delete)
            (
                memory_operations,
                cleaned_content,
            ...
            # If operations are found, process them asynchronously.
            if memory_operations:
                ...
                # This single background task now handles both creation and deletion.
                # Use the dedicated handler to process ops, send citations, and the final status.
                task = asyncio.create_task(
                    self._process_assistant_memories(
                        memory_operations, user_id, **kwargs
                    )
                )
                self._background_tasks.add(task)
                ...
                # Overwrite the response content based on the debug valve.
                if not self.valves.show_assistant_mem_content:
                    assistant_message["content"] = cleaned_content
                    logger.info("Hiding assistant memory content from final response.")
            else:
                logger.info("No assistant-derived memory operations detected.")
                # Explicitly end the status cycle if no ops were found.
                await self._safe_emit(
                    "status", "🏁 Memory processing complete", **kwargs
                )
